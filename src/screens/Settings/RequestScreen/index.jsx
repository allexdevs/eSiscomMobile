      ...
      <ScrollView>
        ...
        <Stack alignItems="center">
          <Select
            ...
            _selectedItem={{
              endIcon: <Icon as={MaterialIcons} size="sm" color="gray.500" mr="4" />,
            }}
            onValueChange={(itemValue) => setPaymentMethod(itemValue)}
          >
            {paymentMethods.map((value) => (
              <Select.Item key={`${value.id}-${value}`} label={value.value} value={value.value} />
            ))}
          </Select>
          <HStack mt="4" alignItems="center" w="90%">
            <Checkbox mt="4" value={checkClients} accessibilityLabel="clientes" />
            <Text mt="4" ml="2">
              Lista todos os Clientes ao abrir o Pedido
            </Text>
          </HStack>
          <HStack alignItems="center" w="90%" mt="4">
            <Checkbox mt="2" value={checkProducts} accessibilityLabel="produtos" />
            <Text mt="2" ml="2">
